...
try:
    OPEN_AI_V1_USED = importlib.metadata.version("openai").startswith("1.")
except importlib.metadata.PackageNotFoundError:
    OPEN_AI_V1_USED = False
    ...
    class OpenAIEmbeddingFunction(EmbeddingFunction):
        ...
        def __call__(self, texts: Documents) -> Embeddings:
            ...
            embeddings = self._client.create(
                input=texts, model=self._deployment_id or self._model_name
            ).data
    ...
    class OpenAIEmbeddingFunction(EmbeddingFunction):
        ...
        def __call__(self, texts: Documents) -> Embeddings:
            ...
            if self._api_type == "azure":
                embeddings = self._client.create(
                    input=texts, engine=self._deployment_id or self._model_name
                )["data"]
            else:
                embeddings = self._client.create(
